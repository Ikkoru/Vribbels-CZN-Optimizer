...
The Overall Stats sheet under the Banners list is `across_pools`, into `Overall`. **The Observe Prism Module is kept apart**: it spends its own currency and pays a 5-star three times as often, so a figure it would dominate is given without it, and its records are its own. The two 50-pull categories never reach a luck figure, having no schedule to compare against.
