...
`default_settings/shared_facts.json` holds facts about the GAME rather than any account's: banner rates, Combatant Trial slot pairings, finished instalments' reward totals, which instalments paid a final reward, and per server the Great Rift's division tops and the Sortie's and the Full-Scale Offensive's fields. It is never copied into `settings/` and never merged: `defaults_sync` does not know it, the app reads it once at startup into `AppContext.shared_facts`, and each reader combines it with the account's own data in memory, the account's own value winning. `shared_facts.py` holds the whitelist, the rules, and the list of readers.
