...
**Its Overall Gacha Stats sheet is a Text, not a list**, because its rows are label rows and one kind sits further apart: `label row -> label row` between rows, four more above a section's heading (`HEADING_PAD`, marked as the exception). A Treeview's rows are one height, with nothing reaching between them. Each line carries one pitch tag, which is what the registry's entries read the two gaps by. The columns are tab stops worked out from the rows on every fill, each only from the rows with something after it. A record's ties follow it as (units, date) pairs: units to their date is `label ↔ its element`, one pair to the next `element and its label ↔ element and its label`. As many pairs as fit its share of the Banners list's width are shown, oldest first, since a wider sheet would widen that column and take the difference from the pulls list.

**The standings stand beside it as lists, not under it as sheets.** Three of them stacked under the gacha sheet run past the default window's bottom edge, and that sheet leaves most of the Banners list's width empty beside it. Each is two Treeviews in the tab's list style: the rows' names, held still, and the seasons, one column each, in a holder that stops at the Banners list's right edge and scrolls sideways past it. A list grows a column a season rather than taking the whole width: held at that width, two seasons would be mostly empty heading over a scrollbar with nothing to scroll. The gacha sheet's ties take at most `OVERALL_SHARE` of the Banners list's width, and the standings have the rest. Each title is a labelwidget, so a grey note on how to fill the list sits on its line (`header subtext`, as the Optimizer's Results title does). `check_tabs_build` fills every list past its room, with the gacha sheet at its widest and a Banners row for every banner type, and measures them in a default-sized window.
