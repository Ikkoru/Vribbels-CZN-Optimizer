...
| block heading and its checkbox run | `ui/tabs/checklist_tab.py`, `BLOCK_PAD` | Extra space on the row that crosses a block's edge -- the first row of a shop or of the Events list, and the first ordinary row after one -- so a block is set apart from what surrounds it. Charged ONCE per edge and only above: paying it below a block as well doubled it wherever two blocks touch, and charging it at the top of a column dropped that column below its neighbours for a gap with nothing on the other side. Open: which rule this is, a heading's own pitch being neither `label row -> label row` nor `panel ↕ unrelated label`. |
