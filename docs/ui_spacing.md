...
**The Sortie Stats and Great Rift Stats sheets under it are the same construct** -- one `_make_sheet` builds all three, on the same levers -- with a figure and a value to a row and no headings, so their first line is a row. They grow a row per season read, and nothing stops a column that grows from running past the window's bottom edge, where the rows cut off leave nothing beside them to look short. So they show the newest `SEASONS_SHOWN` seasons, and `check_tabs_build` fills every sheet to that cap, with a Banners row for every banner type, and measures the column in a default-sized window.
