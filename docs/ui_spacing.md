...
**Every scrollbar in the app is a `ttk.Scrollbar`**, so this one style paints all of them. The Stats & Gacha History tab's derive from it (`GachaHistory.Horizontal.TScrollbar` and its vertical twin): the colours above, a layout with no arrow buttons, and `SCROLLBAR_WIDTH` thick. `scrolledtext.ScrolledText` would not be: it builds its own `tk.Frame` and `tk.Scrollbar` that no constructor keyword reaches, which is why `ui/utils/scrolled_text.py` builds the pair itself instead. See `ui_runtime.md`.
...
| `scrollbar thickness, a style option`               | Stats & Gacha History, the tab's scrollbars | —        | a widget's own size rather than a distance between two things: `SCROLLBAR_WIDTH`, which no gap reads, since every gap to a scrollbar ends at its edge |
