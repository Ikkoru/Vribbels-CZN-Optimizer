...
| `a shop's products under the shop`                  | Checklist, the Weekly and Monthly shop rows | — | a `lmargin1` on the line, in pixels, which Tk draws as the margin itself. Nothing mediates it: no box, no glyph and no geometry manager, so a reading could only report the number back. The `label ↔ its element` stop beside it IS tracked |
