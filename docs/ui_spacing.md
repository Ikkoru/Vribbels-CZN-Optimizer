...
**Every scrollbar in the app is a `ttk.Scrollbar`**, so this one style paints all of them: the colours above, and in both orientations a layout with no arrow buttons, `SCROLLBAR_WIDTH` thick (`czn_optimizer_gui.py`). `scrolledtext.ScrolledText` would not be: it builds its own `tk.Frame` and `tk.Scrollbar` that no constructor keyword reaches, which is why `ui/utils/scrolled_text.py` builds the pair itself instead. See `ui_runtime.md`.
