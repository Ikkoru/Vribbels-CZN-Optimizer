...
- **A checkbox embedded in a Text takes no tags of its own.** `window_create` accepts none, and a line reads `spacing1` and `lmargin1` off its FIRST character — which for such a row is the untagged one the window leaves behind. Tags on the rest of the line style the words and nothing else, so the row silently loses its pitch AND its indent, and moving either changes nothing on screen. Tag the window's own character after inserting it; `check_tabs_build.py` holds it. The box-to-box gap is then exactly `spacing1`, with nothing of the widget's own in it — what the eye sees is that plus the inset the checkbox paints inside its box, twice.
...
| `a shop's products under the shop`                  | Checklist, the Weekly and Monthly shop rows | — | a `lmargin1` on the line, in pixels, which Tk draws as the margin itself. Nothing mediates it: no box, no glyph and no geometry manager, so a reading could only report the number back. It reaches the CHECKBOX only because the window's own character is tagged — see the note above. The `label ↔ its element` stop beside it IS tracked |
...
| checkbox row -> checkbox row | `ui/tabs/checklist_tab.py`, `CHECKBOX_PITCH` | The Checklist's shop products are checkboxes embedded in a Text line, and a checkbox is taller than the line it sits on. Measured: the BOX-to-box gap is exactly the `spacing1`, with nothing of the widget's own in it, so what the eye sees is that plus the inset the checkbox paints inside its box, twice over. `label row -> label row` is read between painted rows and would answer about the box rather than the tick. Open: whether checkbox rows want their own rule or a correction applied to that one. |
