...
| `scrollbar` | `ttk.Scrollbar`                       | its thickness is a style option, `arrowsize`, and a gap to it is read at its trough's edge                                                        |
...
- **A checkbox embedded in a Text takes no tags of its own.** `window_create` accepts none, and a line reads `spacing1`, `lmargin1` and its tab stops off its FIRST character — which for such a row is the untagged one the window leaves behind. Tags on the rest of the line style the words and nothing else, so the row silently loses its pitch, and moving that changes nothing on screen. Tag the window's own character after inserting it; `check_tabs_build.py` holds it. The box-to-box gap is then exactly `spacing1`, with nothing of the widget's own in it — what the eye sees is that plus the inset the checkbox paints inside its box, twice.
...
**The standings stand under it as lists.** The Great Rift's is held at the Banners list's width; under it, the Full-Scale Offensive's and the Sortie's share that width, half each less the pads between them, a title wider than its half taking the difference from its neighbour's list. Each is two Treeviews in the tab's list style: the rows' names, held still, and the seasons, one column each, in a holder that scrolls sideways past its room. The two half lists grow a column a season rather than taking their half: held at it, two seasons would be mostly empty heading over a scrollbar with nothing to scroll. Each title is a labelwidget, so a grey note on how to fill the list sits on its line (`header subtext`, as the Optimizer's Results title does). **A labelwidget `ttk.Label` sits 2px in from a `text=` title** on every side: `TLabel` draws its text inside a 1px border and a 1px padding, where a LabelFrame's own title has neither. So the titles take `TITLE_STYLE`, the title's own layout, and the gap to the note is the note's whole pad. Their columns sit `STANDINGS_COLUMN_GAP` apart, narrower than the Banners and Pulls lists' `COLUMN_GAP`.

**The Banners and Pulls lists share the window's spare width**, half each, in their last columns: the Banners list's by `_fit_banners` on every resize of the tab's body, the Pulls list's by stretching into what its grid column has left. Every column before those keeps its measured width, and the standings keep to the Banners list's width, whatever it has become.
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `Treeview.padding`      | the whole tree area — **and every cell's text inset**, see below                                                                                                                                                                                                                      | **(3, 0, 3, 0)** |
...
| `the settings panel's two columns`                  | Setup & Settings, the `Settings` panel      | —        | the 16px is padding between two frames whose widths are their contents', so the distance the eye meets is that padding only on whichever left-hand row happens to be widest — today the red cores warning, whose wrapping moves with the panel. Every other row reads wider. There is no one gap to hold, and the readings that could be taken answer a different question each time the text rewraps |
...
| `a label's own inset, a style option`               | Stats & Gacha History, the status lines     | —        | the 2px above and below every `TLabel`'s text, kept on these lines so their pitch stays a label's while their sides drop it; a widget's own inset, like a button's |
| `scrollbar thickness, a style option`               | `TScrollbar`, every scrollbar in the app    | —        | a widget's own size rather than a distance between two things: `SCROLLBAR_WIDTH`, which no gap reads, since every gap to a scrollbar ends at its edge |
