...
**Gacha History's two lists take `GachaHistory.Treeview`**, set up in that tab: 12 at each side of a column's text, in cells and headings alike. Its layout has no `Treeview.padding` element either, so its `padding` insets the text alone — widened on the tree area, it would pull the heading row in from both ends and leave a strip of background beside it. Colours and row height fall through to `Treeview`.
