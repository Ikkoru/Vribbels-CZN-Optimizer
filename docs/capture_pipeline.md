...
| `event_defines` | every `event_*_entity` and `event_*_entities` | an event's own record, kept under the key it arrives on. **Every one of them, not a list of the ones in use**: these are what say how BIG an event is — `event_bartender_entities` is one row per day of it — and a table nobody kept cannot be read later, since it rides the login burst and nothing else. A singular carrying a `res_id` is one ROW of its collection and is folded into it; `EVENT_FIELDS_HANDLED` keeps out the tables with a reader of their own |
