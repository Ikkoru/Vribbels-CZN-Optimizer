...
- Gear Score weights reject letters and other irrelevant symbols.
- Repeated "snapshot saved" messages are condensed into one when there is nothing breaking them up.
- Snapshot are no longer read half-written.
...
- The app refreshes it information on every snapshot save.
- Drinking the daily coffee is noticed right away, rather until some other town action mentioned it.
...
- **One text size across the app,** with a second, larger size for status lines. Gear cells put the slot name on the same row as the main stat, and are a little wider.
- **One checkbox style across the app**, with no dotted rectangle when clicked and no Tab focus.
...
- **Tighter, more consistent spacing across every tab,** and panel borders no longer double up around lists that already have one.
...
- **`config.json` is folded into `settings.json`.** `server_region` and `optimizer_workers` move across automatically and the old file is ignored. Editing `optimizer_workers` by hand now needs the program closed first.
...
- **Optimizer tab: the Exclude Combatant's MFs rows are justified**, each full row ending flush with the frame's right edge.
...
- **Polish round: UI and data refinements** — mouse-wheel on every spinbox, type-to-jump on the Optimizer and Combatants dropdowns, fixed-ratio panels in place of draggable splits, set-effect % defaulting to 100, Maximum Flex Slots auto-raised with a notice when the chosen sets cannot leave a valid build, a reorganized Stats Comparison panel, a reformatted Results `Sets` column, a new right-click `Show all stat contributions` breakdown, per-character level 61/62 bonuses in place of global ones, and confirmed exp thresholds for levels 1–40.
- **Polish round 2: Combatants, Optimizer and Memory Fragments refinements** — a fixed-size Combatants detail panel sized from the whole roster so switching combatants never shifts the layout, two-line Build Stats, a wider combatants list, per-column alignment in the Memory Fragments set selector, and alignment work on `Have at least` and Set Configuration.
- **Polish round 3: further UI refinements** — a flow layout for Exclude Combatant's MFs in place of the fixed grid, a note under the `Have at least` grid explaining that its figures are the ones the in-game Combatants menu shows, geometry work across Important Settings and `Have at least`, `+Lv` renamed `Level` with the `+` dropped from level values, a relocated Combatants title row, and a fix for the Optimizer tab settling in view on first open.
...
- **User-state folder renamed `presets/` → `settings/`**, all five files moving together, with bundled defaults in a new `default_settings/` folder. Existing installs migrate on first launch; no data is lost.
- **`config.json` moved into `settings/`**, migrated automatically on first load. Frozen builds resolved it into a read-only temporary directory, silently losing every save.
- **Combatant preset assignments are keyed by id**, with display names kept alongside. An assignment made while a combatant was known only by number stays put when they are properly added. Older files migrate on first load.
- **`OptimizerSettingsManager.load()` preserves unknown top-level keys**, which had been dropped on load.
...
- **Optimizer tab: Stat Priority sliders** — replaced by per-character Important Settings.
- **Optimizer tab: the Main Stats checkbox grid** — replaced by the force-main checkboxes plus an internal per-slot choice.
- **Optimizer tab: the Top % Filter slider** — now an internal performance setting.
- **Optimizer tab: the Include Equipped Items checkbox** — replaced by the per-character exclude list. Equipped fragments are always available except those owned by excluded characters, and the current character's own gear is always available.
- **Optimizer tab: the Reset button** — every setting saves per character, so reset has no clear meaning.
...
- **The active preset moved from `presets.json` to `settings.json`**, being per-user state rather than a shipping default. Legacy files migrate on first load.
...
- The active preset from the bundled `presets.json` defaults; it is per-user state and lives in `settings.json`.
