...
- **A `Gacha History` tab**, right of `Setup & Settings`: every pull the game has listed, kept after the game stops listing it, with each banner type's pity, 50/50s and how its luck compares with the game's published rates. Open each banner's Probability Info → Rescue Records while capturing to add them. `Import JSON` also reads hub-czn's `Export JSON`, re-reading every rarity — its newer 5★ units no longer count as 3★ — and dating each pull back to its banner, so its 50/50s count too.
