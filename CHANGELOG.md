...
- **The optimizer knows the three damage-over-time types apart.** What the program called DoT is the one the game calls Agony, and Important Settings now says so. A second slider under it covers Fracture and Scorched, which the game added later and which behave differently: they can crit and they take buffs, where Agony does neither. Set the two sliders to the share of a combatant's damage each type accounts for. The practical effect is that a combatant whose damage is mostly Agony no longer has crit gear ranked for it, and no longer gains from a conditional set dialled up for its damage multiplier — while one built around Fracture still wants both. Existing combatants keep their setting as their Agony share, and Fracture starts at 0%. Because the sliders ask for a share of *total* damage, and that total is mostly card damage, the Agony share is scaled onto the same footing as the rest — measured against the combatant's own equipped Fragments, falling back slot by slot to the best available one for any slot they have empty. Without that, a heavily-built combatant asking for 50% Agony would have got the weighting of about 20%.
