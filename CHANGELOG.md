...
- **Fracture and Scorched damage.** Important Settings now has three damage-type sliders instead of two: Extra, Agony (what the program used to call DoT) and Fracture. Fracture and Scorched share the one slider. Set each to the share of a combatant's damage it accounts for. Agony can't crit and isn't affected by buffs, so a combatant built around it no longer has crit gear ranked for it. Fracture and Scorched do both. Existing combatants keep their old setting as Agony; Fracture starts at 0%.

- **The Upgraded log lines hide presets that can't use the fragment.** Four new checkboxes in the Capture tab's Upgrade Log Settings. They drop a preset from an Upgraded line when none of its combatants wants that fragment's main stat: an element that isn't theirs, an ATK% main on a DEF-scaling combatant (or the reverse), and HP% or Ego mains for damage dealers. Combatants in the middle of the ATK/DEF split, and those the program doesn't know, are never filtered. All four are on by default and remembered.

### Changed

- **The Combatants tab's character list is a real table now.** It resizes faster, its columns stay lined up, and each row is coloured by its Element. Sorting, scrolling and keyboard navigation match the other lists in the app. New Affinity column, and Partner level has its own column.

- **Combatants tab: Sets and build stats moved into the Character card.** The separate Build Stats strip is gone. Text in the card can now be selected and copied. Total GS is no longer repeated there — the list's GS column already shows it.
...
- **One checkbox style across the app.** They no longer show a dotted rectangle when clicked, and are no longer reachable with Tab.
...
- **Capture tab: "Upgrade Log Presets" is now "Upgrade Log Settings",** it is wider, and its checkboxes sit in five columns.
...
- **Capture missed anything the game sent in a batch.** Most of what the game sends after the loading screen arrives batched, and all of it was being discarded — including the reply that says you obtained a new combatant. That is why a new combatant did not appear until the next time you logged in.

- **Snapshot and hosts-file reads/writes now state their encoding.** On a Japanese or Korean system a snapshot with non-ASCII text in it could fail to load or come back mangled. The hosts file is now left exactly as found.

- **A failure to install the bundled default settings is now reported** instead of leaving the program looking healthy with no presets and no per-combatant settings.
